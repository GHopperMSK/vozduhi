...
class ControllerExtensionTltBlogTltBlog extends Controller {
	...
	protected function getList() {
		...
		if (!$this->config->has('tltblog_status')) {
			$data['error_config'] = sprintf($this->language->get('error_config'), $this->url->link('extension/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
		} else {
			$data['error_config'] = '';
		}
    ...
    public function uninstall()
    {
        ...
        if ($this->config->get('tltblog_tltblog_status')) {
            $this->model_setting_setting->editSettingValue('tltblog', 'tltblog_tltblog_status', '0');
        } elseif ($this->config->has('tltblog_tltblog_status')) {
            $this->model_setting_setting->editSettingValue('tltblog', 'tltblog_tltblog_status', '0');
        }
    }
}